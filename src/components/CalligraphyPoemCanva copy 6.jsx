                ...
                p.draw = () => {
                    ...
                    if (p.mouseIsPressed && drawing) {
                        ...
                        if (isMax) {
                            ...
                            const maxFrames = 10;                          // 幾幀長到最大
                            const t = Math.min(strokeFrame / maxFrames, 1);
                            const threshold = 0.7;                         // 70% 前是慢速區
                            const startRatio = 0.6;                        // 前段最多到 10% 粗度
                            let easedT;

                            if (t < threshold) {
                                // 線性從 0 增長到 startRatio
                                easedT = startRatio * (t / threshold);
                            } else {
                                // 後段 (t∈[0.7,1]) 用二次加速曲線，從 startRatio 加速到 1
